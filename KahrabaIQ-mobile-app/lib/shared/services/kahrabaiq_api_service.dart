...
class KahrabaIqApiService {
  ...
  DashboardData applyScenarioAiResponse(
    ...
    var parsedAi = _parseAiDashboard(aiResult);
    if (parsedAi != null &&
        parsedAi.nextHourEnergyKwh <= 0 &&
        dashboard.reading.power > 0) {
      final fallbackEnergy = dashboard.reading.power / 1000;
      parsedAi = AiDashboardSummary(
        updatedAt: parsedAi.updatedAt,
        source: parsedAi.source,
        modelName: parsedAi.modelName,
        modelVersion: parsedAi.modelVersion,
        inputSource: parsedAi.inputSource,
        energyWaste: parsedAi.energyWaste,
        wasteConfidence: parsedAi.wasteConfidence,
        abnormalUsage: parsedAi.abnormalUsage,
        abnormalUsageConfidence: parsedAi.abnormalUsageConfidence,
        recommendationType: parsedAi.recommendationType,
        statusCode: parsedAi.statusCode,
        statusLabel: parsedAi.statusLabel,
        statusTone: parsedAi.statusTone,
        statusSummary: parsedAi.statusSummary,
        actionTitle: parsedAi.actionTitle,
        nextHourEnergyKwh: fallbackEnergy,
        nextHourCostBhd: fallbackEnergy * dashboard.tariffBhdPerKwh,
        efficiencyScore: parsedAi.efficiencyScore,
        explanation: parsedAi.explanation,
        controlSuggestion: parsedAi.controlSuggestion,
      );
    }

    return dashboard.copyWith(
      aiDashboard: parsedAi,
      ...
      settingsSummary: {
        ...
        'scenario_ai_source_label': 'Generated by backend scenario AI',
  ...
  AiDashboardSummary? _parseAiDashboard(Map<String, dynamic> data) {
    ...
    return AiDashboardSummary(
      ...
      nextHourEnergyKwh: _aiNumericValue(data, [
        'next_hour_energy',
      ...
      nextHourCostBhd: _aiNumericValue(data, [
        'next_hour_cost',
        'next_hour_cost_BHD',
        'next_hour_cost_bhd',
  ...
  double _aiNumericValue(
    Map<String, dynamic> data,
    List<String> flatKeys,
    String predictionKey,
  ) {
    final flat = _pick(data, flatKeys);
    if (flat is Map<String, dynamic>) {
      return _asDouble(_pick(flat, ['value']));
    }
    final flatValue = _asDouble(flat);
    if (flatValue > 0) {
      return flatValue;
    }
    final predictions = _asMap(data['predictions']);
    final nested = _asMap(predictions[predictionKey]);
    return _asDouble(_pick(nested, ['value']));
  }
